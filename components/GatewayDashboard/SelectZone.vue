...
<script>
export default {
  name: 'GatewayDashboardSelectZone',
  ...
  created() {
    if (this.key) {
      let item = this.source.find((item) => item.key === this.key);
      if (item) {
    ...
    filterSource() {
      if (this.filter) {
        if (this.onInit) {
          ...
          return this.source.filter((item) => item.name && item.name.toLowerCase().includes(this.filter.toLowerCase()));
        }
      }
