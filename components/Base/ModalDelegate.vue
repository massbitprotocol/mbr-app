  ...
  <TheModal :open.sync="_visible" :backdrop="true">
    <div class="max-w-[570px] w-full bg-white p-5 rounded-lg z-10 text-left overflow-hidden">
      <div class="w-full flex items-center justify-between">
        <div class="text-heading-2 text-neutral-darkset font-bold">Delegate {{ type }}</div>
        ...
        <form class="w-full flex flex-col items-center" @submit.prevent="handleSubmit(submitDelegate)">
          ...
          >
            <div class="flex-1 flex flex-col gap-2">
              ...
              <div class="mt-5">
                <p v-if="errors[0] || error" class="!font-thin text-red-500 text-xs italic">
                  {{ errors[0] || error }}
                </p>
...
export default {
  ...
  methods: {
    async submitDelegate() {
      ...
      const amount = BigInt(this.form.amount * 1e18);
      const { api } = this.$polkadot;
      const address = this.communityUser.address;
      const staking = api.tx.dapiStaking.stake(stringToHex(this.provider.id), amount);
          ...
          if (status.isFinalized) {
            if (dispatchError) {
              if (dispatchError.isModule) {
                this.error = this.$polkadot.getStakingMessage(dispatchError);
              } else {
                this.error = dispatchError.toString();
              }
            } else {
              ...
              this.$notify({
                type: 'success',
                title: 'Success',
                text: `Staking ${this.provider.name} successfully submitted to block ${blockHash}`,
              });
              this._visible = false;
              this.error = '';
            }
    ...
    async calculateTransactionFee() {
      ...
      // Get transaction fee
      const staking = api.tx.dapiStaking.stake(
